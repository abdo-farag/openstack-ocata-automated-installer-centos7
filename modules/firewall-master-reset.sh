...
then
	...
	iptables -A INPUT -s $osprivatenetwork -m state --state NEW -m tcp -p tcp --dport 8778 -j ACCEPT
...
if [ $nova_in_compute_node == "yes" ]
then
	
	iptables -A INPUT -s $osprivatenetwork -p tcp -m multiport --dports 5900:5999 -j ACCEPT
	iptables -A INPUT -s $osprivatenetwork -m state --state NEW -m tcp -p tcp --dport 16509 -j ACCEPT
	iptables -A INPUT -s $osprivatenetwork -p tcp -m multiport --dports 49152:49215 -j ACCEPT
